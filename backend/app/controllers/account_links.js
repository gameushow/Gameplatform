...
export const getAll = async (req, res) => {
  ...
  try {
    const data = await AccountLinkModel
...
export const create = async (req, res) => {
  const body = req.body
  if (
    _.isUndefined(body.provider_name) ||
    _.isUndefined(body.provider_token) ||
  ...
  try {
    const account = await AccountLinkModel
      .where({
      ...
      .fetch()
    if (account) {
      return res.status(HAS_ACCOUNT.code).json(HAS_ACCOUNT)
    }

    const result = await AccountLinkModel.forge(body).save()
...
export const update = async (req, res) => {
  ...
  try {
    const model = await AccountLinkModel.where('id', id).fetch({ require: true })
    model.set(body)
    await model.save()

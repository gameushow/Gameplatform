...
export const create = async (req, res) => {
  ...
  if (
    ...
    _.isUndefined(body.sell_start_date) ||
    _.isUndefined(body.sell_end_date) ||
  ...
  try {
    const result = await TicketTypeModel.forge(body).save()
...
export const update = async (req, res) => {
  ...
  try {
    const model = await TicketTypeModel.where('id', id).fetch({ require: true })
    model.set(body)
    await model.save()

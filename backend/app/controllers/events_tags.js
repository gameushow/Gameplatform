import EventTagModel from '../models/event_tag'
...
export const getAll = async (req, res) => {
  ...
  try {
    const data = await EventTagModel
...
export const create = async (req, res) => {
  ...
  try {
    const result = await EventTagModel.forge(body).save()
...
export const update = async (req, res) => {
  ...
  try {
    const model = await EventTagModel.where('id', id).fetch({ require: true })
    model.set(body)
    await model.save()

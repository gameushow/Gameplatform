import EventRequiredDataModel from '../models/event_required_data'
...
export const getAll = async (req, res) => {
  ...
  try {
    const data = await EventRequiredDataModel
...
export const create = async (req, res) => {
  const body = req.body
  if (
    _.isUndefined(body.required_data) ||
  ...
  try {
    const result = await EventRequiredDataModel.forge(body).save()
...
export const update = async (req, res) => {
  ...
  try {
    const model = await EventRequiredDataModel.where('id', id).fetch({ require: true })
    model.set(body)
    await model.save()

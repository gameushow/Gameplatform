import CustomfieldAnswerModel from '../models/customfield_answer'
...
export const getAll = async (req, res) => {
  ...
  try {
    const data = await CustomfieldAnswerModel
...
export const create = async (req, res) => {
  const body = req.body
  if (
    _.isUndefined(body.ticket_id) 
  ) {
    return res.status(500).json(REQUIRED)
  }

  try {
    const result = await CustomfieldAnswerModel.forge(body).save()
...
export const update = async (req, res) => {
  ...
  try {
    const model = await CustomfieldAnswerModel.where('id', id).fetch({ require: true })
    model.set(body)
    await model.save()

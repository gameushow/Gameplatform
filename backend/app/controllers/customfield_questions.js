import CustomfieldQuestionModel from '../models/customfield_question'
...
export const getAll = async (req, res) => {
  ...
  try {
    const data = await CustomfieldQuestionModel
...
export const create = async (req, res) => {
  const body = req.body
  if (
    _.isUndefined(body.is_required) ||
  ...
  try {
    const result = await CustomfieldQuestionModel.forge(body).save()
...
export const update = async (req, res) => {
  ...
  try {
    const model = await CustomfieldQuestionModel.where('id', id).fetch({ require: true })
    model.set(body)
    await model.save()
